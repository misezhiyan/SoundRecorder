...
public class SoundRecorder extends JFrame implements MouseListener {
	...
	public SoundRecorder() {
		...
		// 设置窗口上的图标
		Image img = this.getToolkit().getImage(getClass().getResource("/Recorder.jpg"));
	...
	public void save() {
		...
		try {
			// 以当前的时间命名录音的名字
			// 将录音的文件存放到F盘下语音文件夹下
			File filePath = new File("F:/workSpace/eclipse_webservice/SoundRecorder/src/musicTmp");
			...
			// 定义产生后文件名
			tarFile = new File(filePath.getAbsolutePath() + ".mp3");
			SoundExchanger.wavToMp3(file, tarFile);
		...
		public void paint(Graphics g) {
			...
			if (audioDataBuffer != null) {
				...
				for (int i = 0; i < audioDataBuffer.length - 1; ++i) {
					...
					g.drawLine(i * jp2.getWidth() / 256, audioDataBuffer[i] + 200, (i + 1)

							* jp2.getWidth() / 256, audioDataBuffer[i + 1] + 200);
				}
			}
		}
